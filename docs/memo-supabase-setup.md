...
当前项目使用 Supabase 默认邮件服务和默认 `Magic link or OTP` 模板。免费项目未配置自定义 SMTP 时，Dashboard 不允许修改模板，默认邮件只包含一次性登录链接。

跨浏览器登录时，在目标浏览器中打开登录邮件内的链接。每个浏览器会分别建立 Supabase Auth 会话，并通过同一用户和 RLS 策略读取相同数据。
